...
impl GameState {
    pub fn new(num_players: i8) -> GameState {
        ...
        let player_hands = player_indexes
            ...
            .collect();
        let player_scores: Vec<Score> = player_indexes.clone().map(|_| 0).collect();
        ...
        let pawn_colors: Vec<Option<Color>> = pawn_indexes.clone().map(|_| None).collect();
        ...
        match self.turn_state {
            TurnState::PlayingCard => {
                ...
                actions.extend(
                    self.player_hands[player]
                        .iter()
                        .filter(|card| self.can_play_card(player, (*card).clone()))
            ...
            TurnState::DrawingCard => {
                ...
                actions.extend(
                    COLORS
                        .iter()
                        .filter(|color| !self.color_discard_piles[**color as usize].is_empty())
    ...
    pub fn do_action(&mut self, action: Action) {
        ...
        self.turn_state = match action {
            ...
            Action::DrawFromDiscard(color) => self.draw_from_discard(player, color),
            Action::ChoosePawnType(pawn_type) => self.choose_pawn_type(player, pawn_type),
            Action::MoveColor(color) => self.move_color(player, color),
        }
    }

    fn can_play_card(&self, player: PlayerIndex, card: Card) -> bool {
        let maybe_last_card = self.player_color_expeditions[player][card.color as usize].last();
    ...
    fn play_card(&mut self, player: PlayerIndex, card: Card) -> TurnState {
        ...
        let is_expedition_empty =
            self.player_color_expeditions[player][card.color as usize].is_empty();
    ...
    fn draw_from_discard(&mut self, player: PlayerIndex, color: Color) -> TurnState {
        self.player_hands[player].push(self.color_discard_piles[color as usize].pop().expect(
            &format!(
    ...
    fn choose_pawn_type(&mut self, player: PlayerIndex, pawn_type: PawnType) -> TurnState {
        ...
        let pawn = *pawns
            .iter()
            .filter(|pawn| {
                self.pawn_types[**pawn] == pawn_type && self.pawn_colors[**pawn].is_none()
            })
            .nth(0)
            .unwrap();
    ...
    fn move_color(&mut self, player: PlayerIndex, color: Color) -> TurnState {
        ...
        let event = self.color_position_events[color as usize][new_position as usize];
        match event {
    ...
    fn end_game(&mut self) {
        ...
        let max_score = self.player_scores.iter().fold(0, |max_score, score| {
            if *score > max_score {
                *score
            } else {
                max_score
        ...
        self.winner = if potential_winner1.is_some() && potential_winner1 == potential_winner2 {
            potential_winner1
        } else {
            None
        };
    ...
    fn calculate_scores(&mut self) {
        for player in 0..self.player_count() {
            self.player_scores[player] += score_for_artifact_count(self.player_artifacts[player]);
            for pawn in self.player_pawns[player].clone() {
                let position = self.pawn_positions[pawn];
                if position.is_some() {
                    self.player_scores[player] += score_for_position(position.unwrap());
                }
            }
        }
    }
