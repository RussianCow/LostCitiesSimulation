...
impl Display for Action {
    fn fmt(&self, f: &mut Formatter) -> Result {
        let string = match self {
            Action::PlayCard(card) => format!("PlayCard({} {})", card.color, card.number),
            Action::DiscardCard(card) => format!("DiscardCard({} {})", card.color, card.number),
            Action::DrawFromDeck => String::from("DrawFromDeck"),
            Action::DrawFromDiscard(color) => format!("DrawFromDiscard({})", color),
            Action::ChoosePawnType(pawn_type) => format!("ChoosePawnType({})", pawn_type),
